...
class Power_N6705B(GInst):
    ...
    def __init__(self, link, ch) :
        ...
        self.chConvert = {'CH1':'@1', 'CH2':'@2', 'CH3':'@3', 'CH4':'@4', 'ALL':'@1,2,3,4'}
        ...
        try :
            ...
            raise Exception(f'<>< Power_N6705B ><> open 4-CH Power Fail {str(e)}!')


        idn = self.inst.query('*IDN?')

        if 'N6705' not in idn :
            raise Exception(f'<>< Power_N6705B ><> "N6705" not fit in "{idn}"!')
    ...
    def triggerARB(self, ch1 = None, ch2 = None, ch3 = None, ch4 = None):
        ...
        ch_str  = f"@{'1,' if ch1 else ''}{'2,' if ch2 else ''}{'3,' if ch3 else ''}{'4,' if ch4 else ''}"[:-1]     
        chV_str = f"@{'1,' if ch1 else ''}{'2,' if ch2 else ''}{'3,' if ch3 else ''}{'4,' if ch4 else ''}"[:-1]  
        chI_str = f"@{'1,' if isinstance(ch1, str) else ''}{'2,' if isinstance(ch2, str) else ''}{'3,' if isinstance(ch3, str) else ''}{'4,' if isinstance(ch4, str) else ''}"[:-1]    
    ...
    def setTrapezoid(self, V0 = 0, V1 = 0, t0 = 0.1, t1 = 1e-3, t2 = 0.1, t3 = 1e-3, t4 = 0.1) :
        ...
        self.inst.write(f'ARB:VOLT:TRAP:STAR {V0:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:VOLT:TRAP:TOP  {V1:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:VOLT:TRAP:STAR:TIM {t0:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:VOLT:TRAP:RTIM {t1:.6f},({self.chConvert[self.ch]})')        
        self.inst.write(f'ARB:VOLT:TRAP:TOP:TIM {t2:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:VOLT:TRAP:FTIM {t3:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:VOLT:TRAP:END:TIM {t4:.6f},({self.chConvert[self.ch]})')
    ...
    def setTrapezoidI(self, I0 = 0, I1 = 0, t0 = 0.1, t1 = 1e-3, t2 = 0.1, t3 = 1e-3, t4 = 0.1) :
        ...
        self.inst.write(f'ARB:CURR:TRAP:STAR {I0:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:CURR:TRAP:TOP  {I1:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:CURR:TRAP:STAR:TIM {t0:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:CURR:TRAP:RTIM {t1:.6f},({self.chConvert[self.ch]})')        
        self.inst.write(f'ARB:CURR:TRAP:TOP:TIM {t2:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:CURR:TRAP:FTIM {t3:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:CURR:TRAP:END:TIM {t4:.6f},({self.chConvert[self.ch]})')
    ...
    def setRamp(self, V0 = 0, V1 = 0, t0 = 0.1, t1 = 1e-3, t2 = 0.1) :
        ...
        :return: None.
        """          
        self.inst.write(f'ARB:TERM:LAST ON,({self.chConvert[self.ch]})')    # keep last voltage    
        self.inst.write(f'ARB:FUNC:SHAP RAMP,({self.chConvert[self.ch]})')

        self.inst.write(f'ARB:VOLT:RAMP:STAR {V0:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:VOLT:RAMP:END  {V1:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:VOLT:RAMP:STAR:TIM {t0:.6f},({self.chConvert[self.ch]})')
        self.inst.write(f'ARB:VOLT:RAMP:RTIM {t1:.6f},({self.chConvert[self.ch]})')      
        self.inst.write(f'ARB:VOLT:RAMP:END:TIM {t2:.6f},({self.chConvert[self.ch]})')
...
if __name__ == '__main__':
    ...
    ch_str  = f"@{'1,' if ch1 else ''}{'2,' if ch2 else ''}{'3,' if ch3 else ''}{'4,' if ch4 else ''}"[:-1]     
    chV_str = f"@{'1,' if ch1 is True else ''}{'2,' if ch2 is True else ''}{'3,' if ch3 is True else ''}{'4,' if ch4 is True else ''}"[:-1]    
    chI_str = f"@{'1,' if isinstance(ch1, str) else ''}{'2,' if isinstance(ch2, str) else ''}{'3,' if isinstance(ch3, str) else ''}{'4,' if isinstance(ch4, str) else ''}"[:-1]    
