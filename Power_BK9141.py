...
class Power_BK9141(GInst):
    ...
    def __init__(
        self, link="", ch="CH1", excel0=0, ini=0, GP_addr0=0, main_off_line0=0
    ):
        super().__init__()
        """
        ...
        if ini == 1:
            ...
            try:
                ...
                raise Exception(f"<>< Power_BK9141 ><> open 3-CH Power Fail {str(e)}!")
        ...
        if self.sim_inst == 1:
            ...
            while vin_diff > excel0.vin_diff_set or vin_diff < (
                ...
                if vin_ch == 0:
                    ...
                    self.chg_out(act_ch1=excel0.relay0_ch, vset1=vin_new, state1="on")
                    # send the new Vin command for the auto testing channel
                    pass

                elif vin_ch == 6:
                    # self.change_V(vin_new, excel0.relay6_ch)
                    self.chg_out(act_ch1=excel0.relay6_ch, vset1=vin_new, state1="on")
                    # change the vsetting of channel 1 (mapped in program)
                    pass

                elif vin_ch == 7:
                    # self.change_V(vin_new, excel0.relay7_ch)
                    self.chg_out(act_ch1=excel0.relay7_ch, vset1=vin_new, state1="on")
    ...
    def open_inst(self):
        ...
        if self.sim_inst == 1:
            ...
            print(
                "now is open the power supply, in address: "
