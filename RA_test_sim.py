...
cmd_str_I = ("PROG:ISET" + str(act_ch) + ":" + str(vset))
...
if dynamic_NO == 1:
    ...
    while c_board < dynamic_board:
        ...
        # to assign the board number, only change c_board, can put outside
        sh_org_tab.range(
            (y_start + c_dy_board - 1, x_start - 1)).value = c_dy_board

        while c_item < RA_out_V:

            # assign board number
            # sh_org_tab.range((y_start + int(c_dy_board) - 1, x_start - 1)).value = c_dy_board
            sh_org_tab.range(
    ...
    while c_board < total_board:
        ...
        # to assign the board number, only change c_board, can put outside
        sh_org_tab.range((y_start + c_board, x_start - 1)).value = c_board + 1
        while c_item < RA_out_V:
            # sh_org_tab.range((y_start + c_board, x_start - 1)).value = c_board + 1
            sh_org_tab.range(
